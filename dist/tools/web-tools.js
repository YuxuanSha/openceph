    ...
    const webSearch = {
        ...
        async execute(_id, params) {
            ...
            if (cached && Date.now() - cached.ts < CACHE_TTL_MS) {
                ...
                return { content: [{ type: "text", text: cached.results }], details: undefined };
            }
            ...
            try {
                ...
                if (!resp.ok) {
                    return { content: [{ type: "text", text: `Search failed: HTTP ${resp.status}` }], details: undefined };
                }
    ...
    const webFetch = {
        ...
        async execute(_id, params) {
            ...
            try {
                ...
                if (!resp.ok) {
                    return { content: [{ type: "text", text: `Fetch failed: HTTP ${resp.status}` }], details: undefined };
                }
                const html = await resp.text();
                ...
                return { content: [{ type: "text", text: `Fetch error: ${err.message}` }], details: undefined };
            }
        },
    };
    return [
