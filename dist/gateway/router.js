...
export class ChannelRouter {
    ...
    config;
    commandHandler;
    constructor(channels, pairingManager, sessionResolver, messageQueue, brain, config) {
    ...
    async route(msg) {
        ...
        if (policy === "pairing") {
            if (!this.pairingManager.isApproved(msg.channel, msg.senderId)) {
                ...
                await channel.send(this.buildReplyTarget(msg), {
            ...
            try {
                const output = await this.brain.handleMessage({
                    ...
                    onTextDelta: streamHandle
                        ? (delta) => {
                            accumulated += delta;
                            streamHandle.update(accumulated).catch((err) => {
                                gatewayLogger.warn("stream_update_error", { error: err.message });
                            });
                        }
                        : undefined,
                });
                const usedSendToUser = output.toolCalls.some((call) => call.name === "send_to_user" && call.success);
                if (streamHandle) {
                ...
                await typingHandle?.stop().catch((err) => {
    ...
    buildReplyTarget(msg) {
        ...
        return {
            ...
            threadId: typeof message.thread_id === "string" && message.thread_id.trim()
                ? message.thread_id
                : undefined,
            chatId: typeof message.chat_id === "string" && message.chat_id.trim()
                ? message.chat_id
                : undefined,
            metadata: {
