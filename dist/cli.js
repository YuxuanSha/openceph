...
tentacleCmd
    ...
    try {
        ...
        const versionW = 10;
        const runtimeW = 12;
        console.log("  " + "NAME".padEnd(nameW) + "VERSION".padEnd(versionW) + "RUNTIME".padEnd(runtimeW) + "TYPE");
...
tentacleCmd
    ...
    try {
        ...
        const customizable = info.customizable;
...
async function copyDirOverwrite(sourceDir, destDir) {
...
export async function initBuiltinTentacles(targetSkillsDir, skipList = []) {
    ...
    for (const entry of entries) {
        if (!entry.isDirectory() || skipList.includes(entry.name))
            continue;
...
export async function upgradeBuiltinTentacles(targetSkillsDir, skipList = []) {
    const builtinDir = getBuiltinTentaclesDir();
    if (!(await pathExists(builtinDir)))
        return;
    const entries = await fs.readdir(builtinDir, { withFileTypes: true });
    for (const entry of entries) {
        ...
        if (builtinVersion === installedVersion)
            continue;
