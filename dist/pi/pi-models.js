 ...
 */
function resolveProviderApiKey(providerName, config) {
    const profiles = config.auth?.profiles;
    if (!profiles)
        return undefined;
    const order = config.auth?.order?.[providerName];
    if (order) {
 ...
 */
export async function writeModelsJson(modelsJsonPath, config) {
    const providers = {};
    ...
    const piModels = { providers, models: flattenedModels };
