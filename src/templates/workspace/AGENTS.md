...
举例：
  错误："No matching distribution found for openceph-runtime"
  → 理解：pip 在 PyPI 上找不到这个包
  → 判断：这是 OpenCeph 内部包，应该在本地
  → 行动：exec("find ~/.openceph -name 'openceph_runtime' -type d")
  → 找到了 → exec("venv/bin/pip install -e /找到的路径") → 重新 spawn_from_skill
  → 找不到 → 告诉用户"内部依赖包缺失，可能需要重新运行 openceph init"
